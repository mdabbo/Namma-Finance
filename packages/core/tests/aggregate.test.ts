...
/**
 * These cases carry no payments, so every cash component is zero.
 *
 * The valuation is a REQUIRED argument because the old optional fallback
 * computed a non-reconciling one — certificate collections at each
 * certificate's FX snapshot, the rest at the project rate — so the reported
 * components stopped summing to the reported total. See
...
describe("project financials with FX consolidation", () => {
  it("consolidates a USD project into EGP at the stored rate", () => {
    ...
    const fin = computeProjectFinancials(p, [stateOf(c, certs)], [], NO_CASH);
  ...
  it("zero revenue yields zero margin (no division blow-up)", () => {
    const p = project();
    const fin = computeProjectFinancials(p, [stateOf(contract(), [])], [expense()], NO_CASH);
  ...
  it("keeps project cash categories distinct", () => {
    ...
    const fin = computeProjectFinancials(p, [state], [], NO_CASH);
  ...
  it("values historical certificates at their immutable FX snapshots", () => {
    ...
    const fin = computeProjectFinancials(p, [stateOf(c, [oldEgp, currentUsd])], [], NO_CASH);
  ...
  it("sums uncertified value per contract without netting over-certification", () => {
    ...
    const fin = computeProjectFinancials(project(), [overCertified, underCertified], [], NO_CASH);
...
describe("client rollup", () => {
  it("aggregates only the client's own projects", () => {
    const mine = computeProjectFinancials(project({ id: 1, clientId: 7 }), [stateOf(contract(), [cert()])], [], NO_CASH);
    const other = computeProjectFinancials(project({ id: 2, clientId: 8 }), [stateOf(contract({ id: 2 }), [cert({ id: 2 })])], [], NO_CASH);
...
describe("dashboard KPIs", () => {
  it("uses ALL expenses (incl. overhead) for net profit", () => {
    const fin = computeProjectFinancials(project(), [stateOf(contract(), [cert()])], [expense()], NO_CASH);
